...
class PACFile(AudioFile):
    ...
    def ReadFileHeader(self):
        ...
        (sampleRate, nChannels, numSamples, nMDCTLines, nScaleBits, nMantSizeBits)                  = unpack('<LHLLHH',self.fp.read(calcsize('<LHLLHH')))
    ...
    def ReadDataBlock(self, codingParams):
        ...
        for iCh in range(codingParams.nChannels):
            ...
            codingParams.use_MS = pb.ReadBits(MSFLAGBITS)
            scaleFactor=[]
            bitAlloc=[]
        ...
        # using MS
        if codingParams.use_MS == 1:
            
            # Recalculating left and right channels from mid and side channels
            data_MS = []        
            
            # Left = (Mid + Side)*(sqrt(2)/2)
            data_MS.append((np.asarray(data[0]) + np.asarray(data[1]))*(np.sqrt(2)/2)) 
            # Right = (Mid - Side)*(sqrt(2)/2)
            data_MS.append((np.asarray(data[0]) - np.asarray(data[1]))*(np.sqrt(2)/2))
    ...
    def WriteDataBlock(self,data, codingParams):
        ...
        (scaleFactor, bitAlloc, mantissa, overallScaleFactor, codingParams.bitReservoir, codingParams.use_MS) = self.Encode(fullBlockData,codingParams)  # returns a tuple with all the block-specific info not in the file header
        
        # add current mantissa value to mantissa list
        codingParams.mantissa_list.append(mantissa[0].tolist())
        codingParams.mantissa_list.append(mantissa[1].tolist())
        ...
        for iCh in range(codingParams.nChannels):
            ...
            nBytes += MSFLAGBITS # bit for MS flag
            ...
            pb.WriteBits(codingParams.use_MS, MSFLAGBITS) # write use_MS bits
...
if __name__=="__main__":
    ...
    filenames = ["spfe"]#["castanet", "glockenspiel", "harpsichord", "spfe", "spmg"]  
    ...
    #    for Direction in ("Decode"):
            ...
            if Direction == "Encode":
                ...
                codingParams.targetBitsPerSample = 2.9 #( 2.18 -> FOR 96 kb/s, 2.9 -> FOR 128 kb/s, 4.35 -> FOR 192 kb/s) 
                
                codingParams.bitReservoir = 0;
                codingParams.use_MS = 0;
                codingParams.mantissa_list = []
            ...
            # calculate probabilities of mantissa list
            if Direction == "Encode":
                
                # flatten mantissa list
                flat_mantissa_list = [item for sublist in codingParams.mantissa_list for item in sublist]
                
                # count appearances of each value in mantissa list
                counts = []
                for i in range(max(flat_mantissa_list) + 1):
                    counts.append(flat_mantissa_list.count(i))
                    
                # codes
                codes = np.arange(0, max(flat_mantissa_list) + 1).tolist()
                
                # compute probabilities
                probabilities = counts/np.sum(counts)
                
                # compute entropies
                entropy = np.sum(probabilities*np.log2(1/(probabilities + 0.0001)))
                
                table = pd.DataFrame({'code' : codes,
                   'probability (%)' : probabilities*100})
                
                table.to_csv("huffman_table.csv")
